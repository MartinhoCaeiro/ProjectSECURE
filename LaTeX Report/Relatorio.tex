...
% Exemplo prático: offsets e pseudocódigo de parsing (adicionado)
\paragraph{Exemplo prático: offsets e pseudocódigo de parsing}
Para tornar o contrato mais palpável, apresentamos um mapeamento de offsets (em bytes) do início do \textit{frame} e um pseudocódigo simples que demonstra como o \textit{BufferAssembler} deve operar.
...
while socket open:
    ...
    # parse campos fixos do frame (MAGIC, VER, ALG, FLAGS, SEQ, TIMESTAMP, NONCE, TAG, AAD_LEN, CT_LEN)
...
Comentário final: este exemplo mostra por que motivo é crítico não tentar decifrar até o \textit{frame} completo estar disponível e por que a ordem/exatidão dos campos (nonce, AAD, tag) é determinante para a verificação AEAD.
