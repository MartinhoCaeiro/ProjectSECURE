...
Para encriptar, é necessário fornecer também uma chave de encriptação. Nesta fase, a chave de encriptação é "\textit{\textbf{Spartacus}}".
\begin{figure}[h]
    \centering
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{Recursos/ProjectSecure_DB.png}
...
Tambem é possivel ver o estado da ligação WireGuard, caso esteja ligado ou desligado.
\begin{figure}[h!]
	\centering
...
Logo no ínicio da implementação da VPN foi necessário achar uma solução para a configuração de encaminhamento de pacotes, pois o servidor Linux
