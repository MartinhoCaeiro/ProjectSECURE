...
\begin{itemize}
    \item \textbf{BufferAssembler do lado PC}: acumula \textbf{LEN} e corpo até \textit{frame} completo antes de decifrar, evitando leitura parcial ou colagem de mensagens.
    ...
    \item \textbf{Backpressure/ACK opcional}: segunda mensagem só após ACK da primeira em fases de diagnóstico.
...
\begin{itemize}
    \item \textbf{Causa raiz}: \textit{framing} frágil (concatenação sem delimitador) e processamento sem acumulador de \textit{frames}.
    \item \textbf{Mitigações}: \textit{length-prefix} + \textit{BufferAssembler}; não reutilizar (key, nonce); captura e tratamento de exceções sem encerrar o ciclo de leitura.
\end{itemize}
